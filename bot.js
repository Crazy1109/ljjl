...
const adminprefix = "-";
const devs = ['485380710656507914','345923657593257986'];
client.on('message', message => {
...
if (!message.content.startsWith(prefix)) return;
	...
	if (command == "mute") {
		...
		let user = message.mentions.users.first();
		let muteRole = message.guild.roles.find("name", "вαn warning");
		if (!muteRole) return message.reply("** لا يوجد رتبة الميوت 'вαn warning' **").then(msg => {msg.delete(5000)});
